document.addEventListener("DOMContentLoaded", () => {
  ...
  let image = "zoo_buy_logo.jpg"; // Make this uploadable???
  const payments = ["Example 1", "Example 2"]; // How do we get these in a secure way???
  const posted = [{name: "Men's Waterfowl Sweater Vest", qt: 1}, {name: "Men's Jeans", qt: 2}];
  const sold = [{name: "Men's Waterfowl Sweater Vest", qt: 2}, {name: "Men's Jeans", qt: 3}];
  const purchased = [{name: "Used Bike", qt: 1}, {name: "UMass T-Shirt", qt: 5}];
  ...
  for (let x of posted) {
    ...
    // TODO: Add this back once routing/pouchDB issue resolved
